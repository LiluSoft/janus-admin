...
import { IPluginHandle } from "./models/IPluginHandle";
import { ICandidate } from "./index_server";
import { JanusClient } from "./JanusClient";
import { VideoRoomPlugin } from "../plugins";
import adapter from "webrtc-adapter";
import { IWebRTCMedia } from "./models/IWebRTCMedia";
import { JanusExtension } from "./JanusExtension";
import { IEventData } from "../transports/IEventData";
import { ISimulcastBitrate } from "./models/ISimulcastBitrates";
...
import { IEventJSEP } from "../transports/IEventJSEP";
// import "webrtc";
...
export class WebRTC {
	...
	constructor(
		...
		this.janusClient.transport.subscribe_plugin_events(this.janusVideoRoom.session, async (event) => {
	...
	// public async streamsDone(pluginHandle: IPluginHandle, jsep: RTCSessionDescriptionInit | null, media: IWebRTCMedia, rtcConstraints?: any, stream?: MediaStream): Promise<void> {
	// 	if (pluginHandle === null || pluginHandle === undefined ||
	// 		pluginHandle.webrtcStuff === null || pluginHandle.webrtcStuff === undefined) {
	// 		this._logger.warn("Invalid handle");
	...
	// 			stream && stream.getAudioTracks() && stream.getAudioTracks().length) {
	...
	// 				for (const sender of config.pc!.getSenders()) {
	...
	// 						await audioTransceiver.sender.replaceTrack(stream.getVideoTracks()[0]);
	// 					} else {
	...
	// 						await s.replaceTrack(stream.getVideoTracks()[0]);
	// 					}
	// 				}
	// 			} else {
	// 				if (adapter.browserDetails.browser === "firefox" && adapter.browserDetails.version! >= 59) {
	...
	// 	if (!config.pc) {
	// 		const pc_config = { "iceServers": this.iceServers, "iceTransportPolicy": this.iceTransportPolicy, "bundlePolicy": this.bundlePolicy };
	...
	// 		};
	// 		config.pc!.onicecandidate = (event: RTCPeerConnectionIceEvent) => {
	...
	// 					this.sendTrickleCandidate(pluginHandle, { "completed": true });
	// 				} else {
	...
	// 			}
	// 		};
	// 		config.pc!.ontrack = (event: RTCTrackEvent) => {
	...
	// 						config.remoteStream.removeTrack(event.track);
	// 						pluginHandle.onremotestream(config.remoteStream);
	// 					}
	// 				};
	// 			}
	...
	// 		this.registerDataChannelHandlers(pluginHandle);
	// 	}
	...
	// 		await this.createOffer(pluginHandle, null, media);
	// 	} else {
	// 		try {
	...
	// 					this._logger.debug("Adding remote candidate:", candidate);
	// 					// New candidate
	// 					await config.pc!.addIceCandidate(new RTCIceCandidate(candidate));
	// 				}
	// 				// end-of-candidates
	// 				await config.pc!.addIceCandidate({});
	// 				config.candidates = [];
	// 			}
	// 			// Create the answer now
	// 			await this.createAnswer(pluginHandle, media);
	// 		} catch (error) {
	// 			throw error;
	// 		}
	// 	}
	...
	// public async createAnswer(pluginHandle: IPluginHandle, media: IWebRTCMedia, simulcast?: boolean, customizeSdp?: (jsep: RTCSessionDescriptionInit) => void, simulcastMaxBitrates?: ISimulcastBitrate) {
	...
	// public async createOffer(pluginHandle: IPluginHandle, media: IWebRTCMedia, simulcast?: boolean, customizeSdp?: (jsep: RTCSessionDescriptionInit) => void, simulcastMaxBitrates?: ISimulcastBitrate, iceRestart?: boolean) {
	// 	if (!pluginHandle || !pluginHandle.webrtcStuff) {
	// 		this._logger.warn("Invalid handle");
	// 		throw new Error("Invalid handle");
	// 	}
	...
	// 	// throw new Error = (typeof throw new Error === "function") ? throw new Error : webrtcError;
	...
	// 					s.stop();
	// 				} catch (e) {
	// 					this._logger.warn("Error Stopping Stream", s);
	// 				}
	// 			}
	...
	// 		await this.streamsDone(pluginHandle, jsep, media, null, stream);
	// 		return;
	// 	}
	// 	if (this.isAudioSendEnabled(media) || this.isVideoSendEnabled(media)) {
	// 		let constraints: MediaStreamConstraints = {};
	// 		pluginHandle.consentDialog(true);
	// 		let audioSupport: boolean | MediaTrackConstraints = this.isAudioSendEnabled(media);
	...
	// 					try {
	// 						// @ts-ignore
	// 						const screenMedia = await navigator.mediaDevices.getDisplayMedia({ video: true, audio: media.captureDesktopAudio });
	// 						pluginHandle.consentDialog(false);
	// 						if (this.isAudioSendEnabled(media) && !media.keepAudio) {
	// 							const audioStream = await navigator.mediaDevices.getUserMedia({ audio: true, video: false });
	// 							if (stream) {
	// 								// TODO: why is the cast needed?
	// 								(stream as MediaStream).addTrack(audioStream.getAudioTracks()[0]);
	// 							}
	...
	// 						try {
	// 							const screenMedia = await this.getScreenMedia(pluginHandle, constraints);
	// 							pluginHandle.consentDialog(false);
	// 							await this.streamsDone(pluginHandle, jsep, media, null, screenMedia);
	// 						} catch (error) {
	// 							pluginHandle.consentDialog(false);
	// 							throw error;
	// 						}
	...
	// 						let sourceId: string;
	// 						try {
	// 							sourceId = await JanusExtension.getScreen();
	// 						} catch (error) {
	// 							pluginHandle.consentDialog(false);
	...
	// 						};
	// 						try {
	// 							const screenMediaStream = await this.getScreenMedia(pluginHandle, constraints, this.isAudioSendEnabled(media));
	// 							pluginHandle.consentDialog(false);
	// 							this.streamsDone(pluginHandle, jsep, media, null, screenMediaStream);
	// 						} catch (error) {
	// 							pluginHandle.consentDialog(false);
	// 							throw error;
	// 						}
	...
	// 						};
	// 						try {
	// 							const screenMediaStream = await this.getScreenMedia(pluginHandle, constraints);
	// 							pluginHandle.consentDialog(false);
	// 							await this.streamsDone(pluginHandle, jsep, media, null, screenMediaStream);
	// 							// Workaround for https://bugzilla.mozilla.org/show_bug.cgi?id=1045810
	// 							// currentTime no longer exists
	// 							// let lastTime = screenMediaStream.currentTime;
	// 							// const polly = window.setInterval( () =>{
	// 							// 	if (!screenMediaStream)
	// 							// 		window.clearInterval(polly);
	// 							// 	if (screenMediaStream.currentTime === lastTime) {
	// 							// 		window.clearInterval(polly);
	// 							// 		if (screenMediaStream.onended) {
	// 							// 			screenMediaStream.onended();
	// 							// 		}
	// 							// 	}
	// 							// 	lastTime = screenMediaStream.currentTime;
	// 							// }, 500);
	// 						}
	// 						catch (error) {
	// 							pluginHandle.consentDialog(false);
	...
	// 				try {
	// 					const userMediaStream = await navigator.mediaDevices.getUserMedia(gumConstraints);
	// 					pluginHandle.consentDialog(false);
	// 					await this.streamsDone(pluginHandle, jsep, media, null, userMediaStream);
	// 				} catch (error) {
	// 					pluginHandle.consentDialog(false);
	// 					throw error;
	// 				}

...
// declare const Spinner : spin;


const loggerFactory = new BrowserLoggerFactory();
const logger = loggerFactory.create("MAIN", "info");
...
const webrtcClient = new janus.WebRTC(loggerFactory);
let myJanusSignallingConnection: VideoRoomPlugin;
...
$(document).ready(async () => {
	...
	$("#start").one("click", async () => {
		...
		if (!webrtcClient.isWebrtcSupported()) {
		...
		try {
			myJanusSignallingConnection = await VideoRoomPlugin.attach(janusClient);
			...
			$("#start").removeAttr("disabled").html("Stop")
				.click(async () => {
					...
					await myJanusSignallingConnection.dispose();
				});
		} catch (error) {
		...
		// videoPlugin;
		webrtcClient.on_consentDialog(consentDialog);
		webrtcClient.on_mediaState((medium, on) => {
			logger.info("Janus " + (on ? "started" : "stopped") + " receiving our " + medium);
		});
		webrtcClient.on_webrtcState((state, reason) => {
			logger.info("Janus says our WebRTC PeerConnection is " + (state ? "up" : "down") + " now");
			...
			$("#bitrate a").click(async () => {
				const id = $(this).attr("id");
				const bitrate = parseInt(id) * 1000;
				if (bitrate === 0) {
					logger.info("Not limiting bandwidth via REMB");
				} else {
					logger.info("Capping bandwidth to " + bitrate + " via REMB");
				}
				$("#bitrateset").html($(this).html() + "<span class=\"caret\"></span>").parent().removeClass("open");
				await myJanusSignallingConnection.configure({
					request: "configure",
					bitrate
				});
				return false;
		...
		myJanusSignallingConnection.on_joined((event) => {
			const myid = event.plugindata.data.id;
			mypvtid = event.plugindata.data.private_id;
			logger.info("Successfully joined room " + event.plugindata.data["room"] + " with ID " + myid);
			publishOwnFeed(true);
			// Any new feed to attach to?
			if (event.plugindata.data["publishers"]) {
				const list = event.plugindata.data["publishers"];
				...
				for (const publisher of list) {
					...
					logger.debug("  >> [" + id + "] " + display + " (audio: " + audio + ", video: " + video + ")");
					newRemoteFeed(id, display, audio, video);
				}
			}
		});

		myJanusSignallingConnection.on_destroyed(() => {
		...
		myJanusSignallingConnection.on_publishers((event) => {
			const list = event.plugindata.data["publishers"];
			logger.debug("Got a list of available publishers/feeds:");
			logger.debug(list);
			for (const publisher of list) {
		...
		myJanusSignallingConnection.on_leaving((event) => {
			// One of the publishers has gone away?
			const leaving = event.plugindata.data["leaving"];
			logger.info("Publisher left: " + leaving);
			let remoteFeed: IFeed = null;
			for (let i = 1; i < 6; i++) {
		...
		myJanusSignallingConnection.on_unpublished(async (event) => {
			// One of the publishers has unpublished?
			const unpublished = event.plugindata.data["unpublished"];
			logger.info("Publisher left: " + unpublished);
			// @ts-ignore
			if (unpublished === "ok") {
				// That's us
				await myJanusSignallingConnection.dispose();
		...
		myJanusSignallingConnection.on_jsep(event => {
			const jsep = event.plugindata.data.jsep;
			logger.debug("Handling SDP as well...");
			logger.debug(jsep);
			webrtcClient.peerConnection.prepareWebrtcPeer(jsep);// .handleRemoteJsep(jsep);
		...
		webrtcClient.on_localstream((stream) => {
			logger.debug(" ::: Got a local stream :::");
			mystream = stream;
			logger.debug(stream);
			$("#videojoin").hide();
			...
			webrtcClient.attachMediaStream($("#myvideo").get(0) as HTMLVideoElement, stream);
			($("#myvideo").get(0) as HTMLVideoElement).muted = true;
			if (webrtcClient.peerConnection.iceConnectionState !== "completed" &&
				webrtcClient.peerConnection.iceConnectionState !== "connected") {
		...
		webrtcClient.peerConnection.on_cleanup(() => {
		...
		// 					},
		// 					webrtcState(on) {
		// 						logger.info("Janus says our WebRTC PeerConnection is " + (on ? "up" : "down") + " now");
		// 						$("#videolocal").parent().parent().unblock();
		// 						if(!on)
		// 							return;
		// 						$("#publish").remove();
		...
		// 					},
		// 					onmessage(msg, jsep) {
		// 						logger.debug(" ::: Got a message (publisher) :::");
		// 						logger.debug(msg);
		...
		// 								logger.info("Successfully joined room " + msg["room"] + " with ID " + myid);
		// 								publishOwnFeed(true);
		// 								// Any new feed to attach to?
	...
	$("#username").keypress((event) => {
		return checkEnter(event.target, event);
	});
});
...
async function registerUsername() {
	if ($("#username").length === 0) {
		...
		myusername = username;
		await myJanusSignallingConnection.join_publisher({
			request: "join",
			ptype: "publisher",
			room: myroom,
			display: myusername
...
async function publishOwnFeed(useAudio) {
	...
	try {
		const jsep = await webrtcClient.peerConnection.createOffer({
			audioRecv: false,
			videoRecv: false,
			audioSend: useAudio,
			videoSend: true,
...
function toggleMute() {
	let muted = webrtcClient.myStream.isAudioMuted();
	logger.info((muted ? "Unmuting" : "Muting") + " local stream...");
	if (muted)
		webrtcClient.myStream.unmuteAudio();
	else
		webrtcClient.myStream.muteAudio();
	muted = webrtcClient.myStream.isAudioMuted();
...
async function unpublishOwnFeed() {
	...
	await myJanusSignallingConnection.unpublish();
}

async function newRemoteFeed(id, display, audio, video) {
	// A new feed has been published, create a new plugin handle and attach to it as a subscriber
	let remoteFeed :Partial<IFeed>= {};

	let remoteSignallingConnection : janus.VideoRoomPlugin;
	try{
		remoteSignallingConnection = await VideoRoomPlugin.attach(janusClient);
		remoteFeed.janusPlugin = remoteSignallingConnection;
	}catch (e){
		logger.warn("Unable to attach subscriber", e);
	}
	await remoteSignallingConnection.join_subscriber({
		request: "join",
		room: myroom,
		ptype: "subscriber",
		feed: id,
	...
	remoteSignallingConnection.on_attached((event)=>{
		...
		remoteFeed.rfid = event.plugindata.data["id"];
		...
		logger.info("Successfully attached to feed " + remoteFeed.rfid + " (" + remoteFeed.rfdisplay + ") in room " + msg["room"]);
		$("#remote" + remoteFeed.rfindex).removeClass("hide").html(remoteFeed.rfdisplay).show();
	});

	remoteSignallingConnection.on_temporal_substream((event)=>{
		...
		if ((substream !== null && substream !== undefined) || (temporal !== null && temporal !== undefined)) {
			if (!remoteFeed.simulcastStarted) {
				...
				addSimulcastButtons(remoteFeed.rfindex.toString(), remoteFeed.videoCodec === "vp8" || remoteFeed.videoCodec === "h264");
			}
			// We just received notice that there's been a switch, update the buttons
			updateSimulcastButtons(remoteFeed.rfindex.toString(), substream, temporal);
}
	});

	remoteSignallingConnection.on_jsep((event)=>{
	...
	janus.attach(
		{
			...
			success(pluginHandle) {
				...
				logger.info("Plugin attached! (" + remoteFeed.getPlugin() + ", id=" + remoteFeed.getId() + ")");
				logger.info("  -- This is a subscriber");
			...
			onmessage(msg, jsep) {
				logger.debug(" ::: Got a message (subscriber) :::");
				logger.debug(msg);
				const event = msg["videoroom"];
				logger.debug("Event: " + event);
			...
			webrtcState(on) {
				logger.info("Janus says this WebRTC PeerConnection (feed #" + remoteFeed.rfindex + ") is " + (on ? "up" : "down") + " now");
			},
			onlocalstream(stream) {
			...
			onremotestream(stream) {
				...
				WebRTC.attachMediaStream($("#remotevideo" + remoteFeed.rfindex).get(0) as HTMLVideoElement, stream);
				const videoTracks = stream.getVideoTracks();
				if (videoTracks === null || videoTracks === undefined || videoTracks.length === 0) {
					// No remote video
				...
				if (adapter.browserDetails.browser === "chrome" || adapter.browserDetails.browser === "firefox" ||
					...
					bitrateTimer[remoteFeed.rfindex] = setInterval(() => {
						...
						const remoteVideoFeedElement = $("#remotevideo" + remoteFeed.rfindex).get(0) as HTMLVideoElement;
						const width = remoteVideoFeedElement.videoWidth;
						const height = remoteVideoFeedElement.videoHeight;
						if (width > 0 && height > 0)
							$("#curres" + remoteFeed.rfindex).removeClass("hide").text(width + "x" + height).show();
					}, 1000);
				}
